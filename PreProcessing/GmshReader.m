function msh = GmshReader(filename, which)
%
% Gmsh reference file.
% http://www.manpagez.com/info/gmsh/gmsh-2.2.6/gmsh_63.php
%
% MODIFIED FROM THE ORIGINAL CODE AVAILABLE AT:
% https://gitlab.onelab.info/gmsh/gmsh/blob/master/utils/converters/matlab/load_gmsh2.m
% 
% SECSION "Priodic Part of the Mesh" ADDED BY engineerabdullah@ymail.com
...
msh.Types = { ...
    ...
    { 9, 2, 'TRIANGLES9', 'nbTriangles9'}, ... % 20
    { 10, 2, 'TRIANGLES10', 'nbTriangles10'}, ...
    { 12, 2, 'TRIANGLES12', 'nbTriangles12'}, ...
    { 15, 2, 'TRIANGLES15', 'nbTriangles15'}, ...
    { 15, 2, 'TRI15_5thorder', 'nbTri15_5thorder'} ...
...
if strcmp(tline, '$NOD') || strcmp(tline, '$Nodes')
    ...
    fprintf('Syntax error (no $Nodes/$NOD) in: %s\n',  filename);
    fileformat = 0;
end
...
%% Priodic Part of the Mesh. 
% fid = fopen('periodic.msh');
% tline = fgetl(fid);
% while ~strcmp(tline, '$Periodic')
% tline = fgetl(fid);
% end
msh.PER_NODES = zeros(1,2);
tline = fgetl(fid);
if strcmp(tline, '$Periodic')
    tline = fgetl(fid);
    j = 0;
    while ~(strcmp(tline,'$EndPeriodic'))
        tline = fgetl(fid);
        chnm = str2num(tline);
        if (size(chnm,2)==3 && chnm(1)==0)
            fgetl(fid);fgetl(fid);
        elseif (size(chnm,2)==3 && chnm(1)==1)
            n = fscanf(fid, '%d', 1);
            for i = 1 : n
                j = j + 1;
                aux = fscanf(fid, '%d', 2);
